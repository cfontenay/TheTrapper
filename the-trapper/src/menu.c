...
void menuReglages(SDL_Renderer *rendu, SDL_Window *ecran, Images *toutesLesImages)
{
    ...
    while(continuer == 0)
    {
        ...
        if (rep != -2 && rep != 20)      //si il y a un évenements enregistrable et si il a pas appuyé sur valider
            selection = rep;
        if (selection == -1)
            continuer = 1;
...
int renvoieSelectionMenuReglages(int largeurRes, int hauteurRes, Images *toutesLesImages) //renvoue -1 quand on quitte le menu
{
    ...
    while(SDL_PollEvent(&event))
    {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            i = 0;
            while(i <= 10)//10 c'est un chiffre à la rache juste pour dire qu'il y aura 10 resolution possibles
            {
